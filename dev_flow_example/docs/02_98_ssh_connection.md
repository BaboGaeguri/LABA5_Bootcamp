...
- 접속 불가: PC와 Orin 네트워크 도달성 확인 (`ping <orin_ip>`)
...
- 초기에는 `172.16.132.204`로 연결을 시도했지만, 시점에 따라 `ping` 실패 또는 `22/tcp refused`가 발생함.
- Orin에서 SSH 서비스 상태 확인 결과 `openssh-server` 설치됨, `ssh.service active/enabled` 상태였음.
- Orin의 실제 현재 Wi-Fi IP를 확인한 결과 `172.16.138.215`로 변경되어 있었음.
- 해당 IP로 재테스트 시 `ping` 정상, `nc -zv 172.16.138.215 22` 성공 확인.

### 원인

- DHCP 환경에서 Orin IP가 변경되었고, 이전 IP(`172.16.132.204`)를 계속 사용해 접속 실패가 발생함.
...
- user: `laba`
- host ip: `172.16.138.215`
- port: `22`

참고:
- `hostname -I`에 보이는 `172.17.0.1`, `172.18.0.1`은 Docker 브리지 주소이므로 SSH 대상 IP로 사용하지 않음.