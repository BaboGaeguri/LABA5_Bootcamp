...
- ✅ cusparselt: Option A (CUDA 12.6 미지원 확인) / Option B (deb) → **둘 다 불가** (아래 참조)
...
- ✅ torchvision PyPI 설치 시도 블록 §3-b에 추가
...
- [ ] **Orin에서** Option B (NVIDIA deb local) 수동 설치 후 `setup_env.sh` 재실행 ← **Orin 접속 후 수행**
- [ ] 검증: venv activate 없이 `python -c "import torch; torch.cuda.FloatTensor(2).zero_()"` 성공 확인 ← **Orin 접속 후 수행**
...
- [ ] **Orin에 wheel 1회 수동 설치** ← Orin 접속 후 수행
...
- [ ] 설치 검증:
  ...
  print(torchvision.__version__, torchvision.ops.nms(boxes, scores, 0.5))
  "
  ```
- [ ] **setup_env.sh §3-b 정리** — PyPI 0.20.0 시도 블록 제거, 아래 구조로 단순화
  ```bash
  ...
  ```
- [ ] 설치 후 `_TokenizerOnlyProcessor` 제거, `AutoProcessor.from_pretrained()` 복원
...
- [ ] 제거 후 [docs/storage/lerobot_upstream_check/03_orin_lerobot_diff.md](docs/storage/lerobot_upstream_check/03_orin_lerobot_diff.md) 변경 이력 추가
...
- 할 일:
  ...
  - [ ] Orin에서 `nvcc -V` 동작 확인 ← **Orin 접속 후 수행**
...
- [ ] Orin에서 실제 실행하여 검증 통과 확인 ← **Orin 접속 후 수행**
...
| 2 | apt 시스템 패키지 + cusparselt Option A 자동화 + LD 패치 조건화 | §2 | ✅ 스크립트 완료 / Orin 실행 대기 | — |
| 3 | nvcc PATH ~/.bashrc 자동 등록 | §4-1 | ✅ 스크립트 완료 / Orin 실행 대기 | — |
| 4 | 검증 스크립트 강화 (실제 CUDA 텐서 연산) | §5 | ✅ 스크립트 완료 / Orin 실행 대기 | — |
...
| 7 | torchvision **0.20** 설치 (Seeed SharePoint) → `_TokenizerOnlyProcessor` 제거 | §3 | ✅ 결정 완료 / Orin 설치 대기 | 낮음 |
...
**Orin 접속 후 수행:**
- [ ] `setup_env.sh` 재실행 → cusparselt 시스템 설치 확인 + CUDA 텐서 검증 통과 확인
- [ ] `nvcc -V` 동작 확인
- [ ] (권장) Option B: NVIDIA cuSPARSELt deb 수동 설치 → LD_LIBRARY_PATH 패치 완전 제거
- [ ] Seeed SharePoint에서 torchvision **0.20** wheel 브라우저 다운로드 → `pip install`
- [ ] `_TokenizerOnlyProcessor` 제거 → `AutoProcessor.from_pretrained()` 복원 → 동작 확인
- [ ] `ffmpeg -version` 확인 (없으면 `sudo apt install ffmpeg`)
...
- [ ] [docs/storage/lerobot_upstream_check/01_compatibility_check.md](docs/storage/lerobot_upstream_check/01_compatibility_check.md) 에 결과 기록
...
- [ ] **§A-4 Seeed 포크 조사 완료 후** torchvision 설치 여부 확정 → §3 할 일 참조
...
- [ ] Orin에 `ffmpeg` 시스템 패키지 설치 여부 확인 (`ffmpeg -version`). 없으면 `sudo apt install ffmpeg` ← **Orin 접속 후 수행**
