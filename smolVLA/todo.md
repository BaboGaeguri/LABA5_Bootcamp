...
| 9 | **Seeed-Projects/lerobot 포크 Python 3.10 호환 상태 코드 확인** | §A-4 | 포크 채택 시 Orin 패치 3–5개 제거 가능 | 낮음 (조사만) |
...
- [ ] (장기) JetPack 7.x 전환 시 Python 버전 정책 재검토 — 전환 시점에
  Python 3.12 문법 패치 제거 가능 여부를 함께 확인
...
- [ ] https://github.com/Seeed-Projects/lerobot 접속하여 위 5개 항목 **코드 레벨로 확인**
  (설치 없이 GitHub 웹에서 바로 가능)
- [ ] 확인 결과를
  [docs/storage/lerobot_upstream_check/01_compatibility_check.md](docs/storage/lerobot_upstream_check/01_compatibility_check.md)
  에 기록
- [ ] "Seeed 포크 채택 vs 현재 upstream 유지" 결정 및 근거 문서화
- [ ] 교체 결정 시: 서브모듈 URL 변경, `03_orin_lerobot_diff.md`의 패치 중
  포크에 이미 포함된 항목 제거, 재동기화 절차 갱신
